...
//    }
    ...
    private SubProcessDoneWithErrorsPayload buildErrorPayloadFromProcess(Process process) {
        ...
        payload.errors = process.errors.stream()
                .flatMap(error1 -> error1.errors.stream())
                .collect(Collectors.toList());

        // Find the first non-blank exception message and stack trace to report.
        process.errors.stream()
                .filter(error -> StringUtils.isNotBlank(error.exceptionMessage))
                .findFirst()
                .ifPresent(error -> {
                    payload.exceptionMessage = error.exceptionMessage;
                    payload.stackTrace = error.stackTrace;
                });
